...
export function statCellBorderClasses(idx: number): string {
  const isRightEdge2  = idx % 2 === 1;   // last col in 2-col mobile
  const isLastRow2    = idx >= 6;         // row 4 (only cell 6) in mobile
  const isRightEdge4  = idx % 4 === 3;   // last col in 4-col tablet
  const isLastRow4    = idx >= 4;         // row 2 (cells 4-6) in tablet
  return [
    ...
    idx < 6      ? 'lg:border-r lg:border-line-soft' : 'lg:border-r-0',
    'lg:border-b-0',
  ]
    .filter(Boolean)
    .join(' ');
